...
import { Users, AlertTriangle, TrendingUp, Eye, Pencil, Trash2 } from "lucide-react";

import { getCustomers, getStats, deleteCustomer } from "../../services/api";
...
export default function EmployeeDashboard() {
  ...
  const remove = async () => {
    try {
      ...
      setCustomers((prev) => prev.filter((c) => c._id !== selectedCustomer));
    } catch (err) {
      console.log(err);
    } finally {
      setDeleting(false);
  ...
  return (
    <div>
      ...
      <p style={{ color: "var(--ink-muted)", marginTop: 6 }}>Monitor customer churn risk and manage interactions.</p>
                ...
                <td>
                  <div className="action-icons">
                    <div className="tooltip">
                      <button className="icon-btn view-icon" onClick={() => navigate(`/employee/customers/${c._id}`)}>
                        <Eye size={17} />
                      </button>
                      <span className="tooltip-text">View</span>
                    </div>
                    <div className="tooltip">
                      <button className="icon-btn edit-icon" onClick={() => navigate(`/employee/customers/${c._id}/edit`)}>
                        <Pencil size={17} />
                      </button>
