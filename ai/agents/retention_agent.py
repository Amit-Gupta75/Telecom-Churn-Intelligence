...
def _validate(offers) -> bool:
    if not isinstance(offers, list) or not offers:
        return False
    required = {"priority", "category", "title", "description", "riskReduction"}
    return all(isinstance(o, dict) and required.issubset(o.keys()) for o in offers)


def recommend(probability: float, factors: list, profile: dict) -> list:
